  ...
  <div class="post-form-wrapper">
    ...
    <form class="new-post" action="" enctype="multipart/form-data" method="POST">
      ...
      <div class="form-group col-md-12 title-input {{ $errors->has('post_title') ? 'has-error' : '' }}">
        ...
        <input type="text" id="" class="form-control" name="post_title" aria-describedby="Post Title" value="{{ $post->title }}">
        <label class="post-form-error text-danger">{{ $errors->first('post_title') }}</label>
      </div>

      <div class="form-group col-md-6 category-input">
        <label for="">Post Category</label>
        <select class="form-control {{ $errors->has('post_category') ? 'has-error' : '' }}" name="post_category">
          ...
          @foreach($categories as $category )
            <option value="{{$category->id}}"
                @if ($post->category_id == $category->id)
                    selected
                @endif    
            >{{$category->category_genre}}</option>
          @endforeach
        </select>
      ...
      <div class="form-group col-md-8 description-input {{ $errors->has('post_description') ? 'has-error' : '' }}">
        ...
        <textarea class="form-control" rows="7" name="post_description">{{ $post->description }}</textarea>
        <label class="post-form-error text-danger">{{ $errors->first('post_description') }}</label>
      </div>

      <div class="form-group">
        <button type="submit" class="btn btn-light md-banner-btn post-submit-btn"><i class="fa fa-location-arrow"></i> UPDATE POST</button>
